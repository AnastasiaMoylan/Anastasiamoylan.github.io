...
|---|---|---|---|---|
| Employment title, current | `Lead Product Designer \| 2025–Present` (R) | `Lead Experience Designer` called "the employment title" (CS finance comment); B's rule: "Her title is always Lead Product Designer" | CONFLICT | |
...
| Role | `Product Lead and Design Contributor` (P, CS, RW-cwo) | Résumé: `Design Lead and UX / Product Strategy Lead`; SN: `Lead Designer, then Design Lead and UX / Product Strategy Lead` | CONFLICT | |
...
| Shipped | `MVP 1 released and in use; MVP 1.5 in progress` (P, CS) | RW: "+ MVP 2 upcoming"; SN: `Completed, in steady use`; résumé: "completed first MVP … plus a phased roadmap" | CONFLICT | |
...
| Users figure | `70 active users, September 2026` — own count, not project record (F, CS) | SN: `70 returning users … return window not defined`; recharacterised 2026-09-09 without a new source | CONFLICT | |
| Backlog figure | `Cleared a backlog of a few hundred billing packages` — own account (CS, F) | B: "unblocked recovery … volume itself unverified"; SN: size "lost, to recover from PO or program records" | CONFLICT | |
...
| Role | `Product Experience Lead` (P, CS) | Résumé: `Lead UX / Product Designer`; RW-fc: `Lead Product Designer`; CS comment calls this "the fifth title in play" | CONFLICT | |
...
| Shipped | `V3 of the analysis platform in testing; unified homepage MVP in development` (P, CS) | SN: `Active; V3 in testing`; RW: `Working POC delivered; scaled pilot in progress` | CONFLICT | |
...
| Pilot arc | `10 pilot users → 300 → 1,000+ planned` — own account, restored 2026-09-09 on instruction; what 300 counts is unconfirmed; 1,000+ is a plan (F, CS) | — | AGREE (as caveated) | |
...
| Role | `Lead Experience Designer — UX and product strategy lead` (P, CS, RW-doc) | Résumé and SN: `UX and Product Strategy Lead` | CONFLICT | |
...
| Team size | — | — | MISSING | |
| Shipped | `Multi-phase accelerator and product development; production status to confirm` (P, CS, RW) | SN: "shipment and adoption not verified"; what is evidenced is specification to acceptance-criteria depth, not shipment | OPEN | |
