...
## Update — 2026-09-19: drawn visuals replace the screenshots (branch `claude/portfolio-design-review-wivorc`)

Owner's brief: "Create modern visuals for the case studies. I dont like the ones I have supplied."

**What changed.** Every screen on the site is now drawn, not photographed: eleven SVG figures generated by `scripts/visuals/` (`lib.mjs` primitives, `scenes.mjs` one function per figure, `build.mjs` writes them to `src/assets/case-studies/`). Four covers, one per study, double as each study's opener plate (Finance Cloud, Document AI and Customer Journey had no opener before); the billing report, package index and review flow, and the four Customer Journey screens are redrawn as plates. Each figure is an anonymised schematic of the real product in the site's tokens: generic vocabulary (project number, owner, status), invented but internally consistent numbers, the human-review gate in maroon wherever the product has one. This also closes the NDA questions on the flow panels (feedback-queue C3) and gives Document AI a visual it could not have from screenshots.
