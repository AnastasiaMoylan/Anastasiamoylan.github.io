 ...
 * outside the work itself. A block the record cannot support is absent, and
 * the count in the lead sentence says so.
 *
 * Each term is tinted to the band it names in the ownership diagram that
 * follows, and the convention is announced once in a sentence rather than
 * drawn as a legend. It replaces the look-back a reader would otherwise do
 * between the prose and the figure, and it is the device the reference
 * research named as the highest-value one available to an enterprise case
 * study.
 *
 * Colour is never the only signal: every term also carries an underline, so
 * the pairing survives colour blindness and a greyscale print.
 */
