 ...
 *
 * Studies still carrying the pre-2026-09-09 paragraph render it unchanged, so
 * copy migrates on sign-off rather than on the type.
 */
...
export default function OverviewSection({ overview }: { overview: string | OverviewLines }) {
  ...
  return (
    <dl className="m-0 flex measure flex-col gap-5">
      {LINES.map(({ key, label }) => (
        <div key={key}>
          <dt className="m-0 font-mono text-[0.6875rem] font-medium uppercase tracking-[0.12em] text-tertiary-700">
            {label}
          </dt>
          <dd
            className={[
              "mt-1.5 m-0 text-[1.0625rem] leading-[1.65]",
